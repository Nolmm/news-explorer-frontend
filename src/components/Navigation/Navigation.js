  ...
  return (
    <nav className={` navigation ${props.isOpenMob ? 'navigation__mobile-opened' : props.theme ? 'navigation__black' : ''}`}>
      <ul className="navigation__items">
        ...
        <li>{props.loggedIn ?
          <button className={`navigation__button ${props.theme ? 'navigation__button_border-black' : 'navigation__button_border-white'}`}>{'Грета'} {props.theme ? <svg className="navigation__logaut navigation__logaut_mobile" width="24" height="24" fill="none" xmlns="http://www.w3.org/2000/svg">
